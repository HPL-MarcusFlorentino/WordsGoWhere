...
| Image hit area only registers on a tiny corner of the visible sprite | Phaser evaluates custom `setInteractive(rect, Rectangle.Contains)` rects in the image's **local (pre-scale) coordinate space**, then composes the image's `scaleX/Y` (set by `setDisplaySize`) on top during hit testing. If the rect is sized in *display* pixels (e.g. `sd(w) × sd(h)`), Phaser scales it down a second time — producing a world hit region roughly `scale²` of what you expect, pinned near the origin. Two correct options: (1) size the rect in source-texture pixels (`frame.width × frame.height`, centered via `-w/2, -h/2` for origin 0.5); or (2) just call `setInteractive()` with no args — Phaser's default hit area is the full texture bounds, which after the image's transform exactly matches the visible sprite. Avoid making a Container the interactive target as a workaround: `container.setSize(w, h)` anchors its implicit hit bounds at top-left, not center, so a centered rect mismatches what Phaser tests. When in doubt, draw the hit area live with a debug `Graphics` at `m.tx + r.x * m.scaleX` to see exactly what Phaser is testing against. See [TileInteraction.ts](src/game/TileInteraction.ts) `applyHitArea()`. |
